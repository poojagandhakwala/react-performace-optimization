...
export const PERFORMANCE_MODULES: OptimizationModule[] = [
  {
    ...
    description: 'Learn how to use React.memo, useMemo, and useCallback to avoid unnecessary updates.',
    whyMatters: 'By default, React re-renders all children when a parent state changes. In large trees, this causes noticeable lag.',
    whenToUse: "When you have leaf components with many props, or expensive computations that don't need to run every render.",
...
function Parent() {
  ...
  },
  {
    ...
    description: 'Render only what is visible to the user, saving memory and CPU.',
    whyMatters: 'DOM nodes are expensive. Rendering 10,000 items at once can crash a browser tab or cause extreme jank.',
    whenToUse: 'Any list longer than 100-200 items, especially if items are complex.',
...
function Search() {
  ...
  },
  {
    ...
    whyMatters: 'If a render includes a 10ms calculation, and it re-runs 100 times, you lose 1 second of main thread time.',
    whenToUse: 'Filtering, sorting, data transformations, or any loop over large sets.',
...
function Stats({ items }) {
  ...
  },
  {
    ...
    description: "De-prioritize a piece of state so it doesn't block more urgent updates.",
    whyMatters: "Similar to transitions, but for data passed down from parents that you don't control.",
    whenToUse: 'When you want to show a "stale" version of UI while the fresh one is loading.',
